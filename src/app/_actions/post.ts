...
import { formatDateForSQL, mapIndices } from "@/lib/utils";
...
export const basicVerification = async (
  ...
  const getCommunes = regions.find((r) => r.region === data.city)?.comunas as string[]
  ...
  try {
    ...
    const cleanedData = {
        ...
        price: setFromRange([data.minPrice, data.maxPrice]),
        costs: setFromRange([data.minCost, data.maxCost]),
    } as Post;
